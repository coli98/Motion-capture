...
#define TCAADDR 0x70
uint8_t ch;
int tablicaKanalow[] = {0, 1, 2, 3, 4, 5, 6, 7};
int wskaznikTablicy = 0;
...
/****************************************************/
  for (wskaznikTablicy=0; wskaznikTablicy<8; wskaznikTablicy++)  // multiple I2C devices
  {
    Serial.print("Sprawdzam kanal ");
    Serial.print(wskaznikTablicy);
    Serial.print(":   ");
    tcaselect(wskaznikTablicy);
 
    /* Initialise the sensor */
    if(!bno.begin())
    {
      /* There was a problem detecting the BNO055 ... check your connections */
      Serial.println("Kurła no nie ma tego gunwa, gdzies to wpial?");
      delay(500);
      tablicaKanalow[wskaznikTablicy] = 99;
    }
    else
    {
      Serial.println("Tu jestes, swirusku ty jeden!");
    }
  }
...
void loop(void)
{
  ...
  for (ch=0; ch<8; ch++)  // multiple I2C devices
  {
    if (tablicaKanalow[ch] == 99)
    {
      ch++;
...
//      Serial.print(F("Euler: "));
    //*****************************************************
            ...
            Serial.print(euler.x());Serial.println(F(""));  // heading, nose-right is positive, z-axis points up
